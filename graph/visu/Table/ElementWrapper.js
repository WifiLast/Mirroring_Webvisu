	...
	TableElementWrapper.prototype = {
		...
		_processAlarms: function (value) {
			...
			try {
				...
				console.log("TableElementWrapper._processAlarms: Extracted entries (Length: " + entries.length + ")", entries);
				...
				if (entries.length === 1 && Array.isArray(entries[0]) && entries[0].length > 5) {
					console.log("TableElementWrapper._processAlarms: Unwrapping nested array...");
					entries = entries[0];
				}
			...
			console.log("TableElementWrapper._processAlarms: Final Filtered Length: " + entries.length);
